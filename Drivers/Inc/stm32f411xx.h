...
/*
 * ARM Cortex M4 processor NVIC Interrupt Priority Registers from NVIC_IPR0 - NVIC_IPR59 address
 */
...
/*
 * @IRQPriority
 * IRQ (Interrupt Request) Priority
 */
#define IRQ_PRIO_0			0
#define IRQ_PRIO_1			1
#define IRQ_PRIO_3			3
 ...
 */
#define SYSCFG_PCLK_EN()	(RCC->APB2ENR |= (1 << 14))	/* GPIOA Peripheral clock Enable*/
...
#define GPIOH_PCLK_EN()	(RCC->AHB1ENR |= (1 << 7))	/* GPIOE Peripheral clock Enable*/

/*
 * Reset GPIOx
 * *by reset then clear bit in RCC_AHB1RSTR
 */
...
#define GPIOH_REG_RESET()	do { (RCC->AHB1RSTR |= (1 << 7)); (RCC->AHB1RSTR &= ~(1 << 7)); } while(0)	/* GPIOE Peripheral register reset*/

/*
 * Return port code for given GPIOx base address
 */
#define GPIO_BASEADDR_2_CODE(GPIOx)		(	(GPIOx == GPIOA) ? 0 : \
											(GPIOx == GPIOB) ? 1 : \
											(GPIOx == GPIOC) ? 2 : \
											(GPIOx == GPIOD) ? 3 : \
											(GPIOx == GPIOE) ? 4 : \
